...
let backendBridgePort=null;
let backendBridgePromise=null;
const backendBridgePending=new Map();

function bridgeRequestId(){
  return `mfbridge_${Date.now()}_${Math.random().toString(36).slice(2,10)}`;
}
function bridgeUrl(){
  const u=new URL(String(CONFIG.API_URL).trim());
  u.searchParams.set('bridge','1');
  u.searchParams.set('_v','311');
  return u.toString();
}
function resetBackendBridge(){
  try{backendBridgePort?.close?.();}catch{}
  backendBridgePort=null;
  backendBridgePromise=null;
  backendBridgePending.forEach(p=>clearTimeout(p.timer));
  backendBridgePending.clear();
  if(backendBridgeFrame){backendBridgeFrame.remove();backendBridgeFrame=null;}
}
function ensureBackendBridge(timeoutMs=20000){
  if(backendBridgePort)return Promise.resolve(backendBridgePort);
  if(backendBridgePromise)return backendBridgePromise;
  if(!isConfigured())return Promise.reject(new Error('Backend is not configured. Check config.js.'));

  backendBridgePromise=new Promise((resolve,reject)=>{
    let settled=false;
    const frame=document.createElement('iframe');
    backendBridgeFrame=frame;
    frame.src=bridgeUrl();
    frame.title='MyFinance backend bridge';
    ...
    const cleanupListener=()=>window.removeEventListener('message',onReady);
    const fail=message=>{
      if(settled)return;
      settled=true;
      cleanupListener();
      ...
      if(backendBridgeFrame===frame)backendBridgeFrame=null;
      reject(new Error(message));
    };
    const onReady=event=>{
      const data=event.data||{};
      if(!data.__myfinanceBridgeReady)return;
      const port=event.ports?.[0];
      if(!port)return;
      settled=true;
      cleanupListener();
      clearTimeout(timer);
      backendBridgePort=port;
      backendBridgePromise=null;

      backendBridgePort.onmessage=bridgeEvent=>{
        const msg=bridgeEvent.data||{};
        if(!msg.__myfinanceBridgeResponse||!msg.requestId)return;
        const pending=backendBridgePending.get(msg.requestId);
        if(!pending)return;
        backendBridgePending.delete(msg.requestId);
        clearTimeout(pending.timer);

        if(!msg.ok){
          pending.reject(new Error(msg.error||'Backend bridge call failed.'));
          return;
        }
        let result;
        try{result=JSON.parse(String(msg.raw||''));}
        catch{pending.reject(new Error('The backend returned an unreadable bridge response.'));return;}
        if(!result.ok){
      ...
      backendBridgePort.onmessageerror=()=>{
        resetBackendBridge();
      };
      backendBridgePort.start?.();
      resolve(backendBridgePort);
    };

    window.addEventListener('message',onReady);
    frame.addEventListener('error',()=>fail('The Apps Script bridge could not be loaded.'));
    const timer=setTimeout(()=>fail('The backend bridge did not start. Confirm the Apps Script Web App is deployed as Execute as Me / Anyone.'),timeoutMs);
    document.body.appendChild(frame);
  });
  return backendBridgePromise;
}
async function apiViaBridge(action,payload={},options={}){
  const timeoutMs=Number(options.timeoutMs)||CONFIG.REQUEST_TIMEOUT_MS||60000;
  const port=await ensureBackendBridge(Math.min(timeoutMs,20000));
  ...
  return new Promise((resolve,reject)=>{
    const timer=setTimeout(()=>{
      ...
      reject(new Error('The backend request timed out. Please try again.'));
    },timeoutMs);
    backendBridgePending.set(requestId,{resolve,reject,timer});
    try{
      port.postMessage({__myfinanceBridgeRequest:true,requestId,payload:body});
    }catch(e){
      clearTimeout(timer);
      backendBridgePending.delete(requestId);
      resetBackendBridge();
      reject(new Error(`Could not send request to backend bridge: ${e.message}`));
    }
  });
}
async function api(action,payload={},options={}){
  try{
    return await apiViaBridge(action,payload,options);
  }catch(error){
...
async function loadBackendVersion(){
  ...
  try{
    ...
    if(data&&data.version){
      ...
      if(parts[0]<3||(parts[0]===3&&parts[1]<11)){
        showRuntimeWarning(`Backend v${state.backendVersion} is older than required v3.11.0. Update the Apps Script backend before using V18.7.`);
      }
    }
  }catch(e){
...
console.info('MyFinance v18.7 loaded — persistent Apps Script bridge transport');
