...
function detailDate(value){
  ...
  return Number.isNaN(d.getTime())?escapeHtml(raw):new Intl.DateTimeFormat('en-IN',{day:'2-digit',month:'short',year:'numeric'}).format(d);
}
...
function notesBaseItems(){
  if(els.notesScope?.value==='CURRENT')return visibleHoldings();
...
function renderNotesModal(){
  ...
  let items=notesBaseItems().filter(h=>{
    const has=Boolean(String(h.notes||'').trim());
    if(filter==='WITH_NOTES'&&!has)return false;
    if(filter==='WITHOUT_NOTES'&&has)return false;
    const hay=`${h.owner} ${h.type} ${h.assetName} ${h.code} ${h.notes||''}`.toLowerCase();
  ...
  const withNotes=items.filter(h=>String(h.notes||'').trim()).length;
  if(els.notesSummary){
    const scope=els.notesScope?.value==='CURRENT'
      ? `${state.selectedOwner==='ALL'?'Combined':shortOwner(state.selectedOwner)} · ${assetViewLabel()}`
      : 'Entire portfolio';
    els.notesSummary.textContent=`${scope} · ${items.length} investment${items.length===1?'':'s'} · ${withNotes} with personal notes`;
  }
  els.notesList.innerHTML=items.map(h=>{
    const note=String(h.notes||'').trim();
    return `<article class="investment-note-card ${note?'has-note':'no-note'}">
      <div class="note-card-top">
        <div class="asset-cell">
          <div class="asset-badge ${String(h.type).toLowerCase()}">${escapeHtml(h.type==='MF'?'MF':h.type==='ETF'?'ET':'ST')}</div>
          <div><strong>${escapeHtml(h.assetName)}</strong><span>${escapeHtml(shortOwner(h.owner))} · ${escapeHtml(h.type)} · ${escapeHtml(h.exchange?`${h.exchange}:`:'')}${escapeHtml(h.code)}</span></div>
        </div>
        <strong class="note-card-value">${formatCurrency(h.currentValue,true)}</strong>
      </div>
      <div class="note-card-body ${note?'':'empty'}">${note?escapeHtml(note):'No personal note added yet.'}</div>
      <div class="note-card-actions">
        <button type="button" class="small-button" data-note-view="${escapeHtml(h.id)}">View investment</button>
        <button type="button" class="small-button" data-note-edit="${escapeHtml(h.id)}">${note?'Edit note':'Add note'}</button>
      </div>
    </article>`;
  }).join('');
