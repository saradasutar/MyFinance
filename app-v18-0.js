...
function saveTableSize(section,size){
  try{localStorage.setItem(tableSizeKey(section),JSON.stringify({row:clampTableScale(size.row),width:clampTableScale(size.width)}));}catch{}
}
...
function saveColumnWidths(section,widths){
  try{localStorage.setItem(tableColumnWidthKey(section),JSON.stringify(widths||{}));}catch{}
}
...
function resetTableSize(section){
  ...
  applyTableSize(section);
  toast(`${section==='holdings'?'Holdings':'Watchlist'} table size and dragged columns reset.`,'success');
}
...
console.info('MyFinance v18.0 loaded — auto-moving quote carousel');
