...
function renderLifeQuote(resetForDay=false){
  ...
  if(!state.lifeQuotePaused&&items.length>1)startLifeQuoteShuffle();
}
...
function startLifeQuoteShuffle(){
  stopLifeQuoteShuffle();
  if(state.lifeQuotePaused||state.lifeQuotes.length<2)return;
  state.lifeQuoteTimer=setInterval(()=>{
    if(document.hidden||state.activeSection!=='overview')return;
    state.lifeQuoteIndex=normalizeLifeQuoteIndex(state.lifeQuoteIndex+1);
    renderLifeQuote(false);
  },30000);
}
...
function switchSection(section){
  ...
  if(section==='overview')renderLifeQuote(false);else stopLifeQuoteShuffle();
  scheduleDashboardHScrollRefresh();
}
...
console.info('MyFinance v16.7.1 loaded — forced frontend cache refresh');
...
document.addEventListener('visibilitychange',()=>{if(!document.hidden&&state.activeSection==='overview'&&!state.lifeQuotePaused)startLifeQuoteShuffle();});
