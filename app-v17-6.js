...
function setOverviewMode(mode,{saveDefault=false}={}){
  ...
  if(!investment)renderPersonalHome();
...
function renderLifeQuote(resetForDay=false){
  ...
  els.lifeQuoteShuffleStatus.textContent=items.length<2?'1 saved quote':state.lifeQuotePaused?'Auto shuffle paused':`Auto shuffle · 30s · ${items.length} saved`;
  if(!state.lifeQuotePaused&&items.length>1&&state.utilityDrawerOpen&&state.utilityDrawerTab==='QUOTE')startLifeQuoteShuffle();
}
...
function startLifeQuoteShuffle(){
  stopLifeQuoteShuffle();
  if(state.lifeQuotePaused||state.lifeQuotes.length<2||!state.utilityDrawerOpen||state.utilityDrawerTab!=='QUOTE')return;
  state.lifeQuoteTimer=setInterval(()=>{
    if(document.hidden||!state.utilityDrawerOpen||state.utilityDrawerTab!=='QUOTE')return;
    state.lifeQuoteIndex=normalizeLifeQuoteIndex(state.lifeQuoteIndex+1);
    renderLifeQuote(false);
  },30000);
}
...
function setHoldingSummaryCard(valueId,metaId,items){
  const v=holdingsSummaryValues(items);
  if(els[valueId])els[valueId].textContent=formatCurrency(v.invested,true);
  if(els[metaId]){
    const gainText=`${v.gain>=0?'+':'-'}${formatCurrency(Math.abs(v.gain),true)}`;
    els[metaId].textContent=`Current ${formatCurrency(v.current,true)} · Gain ${gainText} · ${v.count} item${v.count===1?'':'s'}`;
    els[metaId].classList.toggle('positive',v.gain>0);
    els[metaId].classList.toggle('negative',v.gain<0);
  }
}
function renderHoldingsSummary(){
  ...
  setHoldingSummaryCard('holdSumCombinedTotal','holdSumCombinedTotalMeta',all);
  setHoldingSummaryCard('holdSumCombinedMf','holdSumCombinedMfMeta',holdingSummarySubset(all,'MF'));
  setHoldingSummaryCard('holdSumCombinedStock','holdSumCombinedStockMeta',holdingSummarySubset(all,'STOCKS'));

  setHoldingSummaryCard('holdSumSaradaTotal','holdSumSaradaTotalMeta',sarada);
  setHoldingSummaryCard('holdSumSaradaMf','holdSumSaradaMfMeta',holdingSummarySubset(sarada,'MF'));
  setHoldingSummaryCard('holdSumSaradaStock','holdSumSaradaStockMeta',holdingSummarySubset(sarada,'STOCKS'));

  setHoldingSummaryCard('holdSumNiharikaTotal','holdSumNiharikaTotalMeta',niharika);
  setHoldingSummaryCard('holdSumNiharikaMf','holdSumNiharikaMfMeta',holdingSummarySubset(niharika,'MF'));
  setHoldingSummaryCard('holdSumNiharikaStock','holdSumNiharikaStockMeta',holdingSummarySubset(niharika,'STOCKS'));
}
...
console.info('MyFinance v17.6 loaded — Holdings combined/Sarada/Niharika investment summary');
