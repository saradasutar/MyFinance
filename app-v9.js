...
function renderWatchlist(){
  ...
  els.watchBody.innerHTML=items.map(x=>`<tr class="watch-row" data-view-watch="${escapeHtml(x.id)}" tabindex="0" aria-label="View watchlist details for ${escapeHtml(x.assetName)}">
    ...
    <td class="row-actions"><button class="small-button" data-view-watch-button="${escapeHtml(x.id)}">View</button><button class="small-button" data-edit-watch="${escapeHtml(x.id)}">Edit</button><button class="small-button danger" data-delete-watch="${escapeHtml(x.id)}">Delete</button></td>
  </tr>`).join('');
...
function openWatchDrawer(item){
  ...
  els.drawerContent.innerHTML=`
    ...
    <section class="drawer-section">
      ...
      <div class="drawer-notes ${item.notes?'':'drawer-notes-empty'}">${item.notes?escapeHtml(item.notes):'No personal note added yet. Use Edit watch item to add one.'}</div>
    </section>
  `;
  els.holdingDrawer.classList.add('open');
  els.holdingDrawerBackdrop.classList.add('open');
