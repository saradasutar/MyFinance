...
const state = {
  ...
  overviewMode: localStorage.getItem('portfolio_overview_default') || 'PERSONAL',
  diary: [],
  monthlyDiary: [],
  monthStatus: [],
  diaryView: 'DAILY',
...
function overviewDefaultMode(){
  return localStorage.getItem('portfolio_overview_default')==='INVESTMENT'?'INVESTMENT':'PERSONAL';
}
function setOverviewMode(mode,{saveDefault=false}={}){
  state.overviewMode=mode==='INVESTMENT'?'INVESTMENT':'PERSONAL';
...
function switchSection(section){
  ...
  if(section==='overview')setOverviewMode(state.overviewMode);
  scheduleDashboardHScrollRefresh();
}
...
console.info('MyFinance v17.2 loaded — personal home + larger fonts + alternate colors');
