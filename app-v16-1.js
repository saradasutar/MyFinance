...
async function init(){bindEvents();loadBackendVersion();els.diaryBrowseDate.value=localIsoDate();els.diaryBrowseMonth.value=localIsoMonth();els.diaryDate.value=localIsoDate();els.monthlyEntryMonth.value=localIsoMonth();els.quickDailyDate.value=localIsoDate();els.quickMonthlyMonth.value=localIsoMonth();updateQuickDiaryCounts();refreshMonthlyYearFilter();els.monthlyYearFilter.value=String(new Date().getFullYear());els.monthlyMonthFilter.value=String(new Date().getMonth()+1).padStart(2,'0');loadSavedUsername();refreshOwnerControls();updateDiaryWritingMeta();setTimeout(()=>{restoreDailyDraft();restoreMonthlyDraft();scheduleDashboardHScrollRefresh();},80);startAutoRefresh();if(!isConfigured()){els.loginMessage.textContent='Setup required: paste the Apps Script /exec URL into config.js.';return;}if(state.token){showApp();loadCache();await loadDashboard(false);resetAutoRefreshClock();}}
console.info('MyFinance v16.1 loaded — layout polish + fully visible quick diary');
init();
