...
function renderAll(){refreshAssetViewControls();renderSummary();renderGrowthDashboard();renderTypeSummary();renderAllocation();renderInvestorSummary();renderTopHoldings();renderHoldings();renderWatchlist();if(state.user?.role==='ADMIN')renderUsers();}
...
async function init(){bindEvents();refreshOwnerControls();startAutoRefresh();if(!isConfigured()){els.loginMessage.textContent='Setup required: paste the Apps Script /exec URL into config.js.';return;}if(state.token){showApp();loadCache();await loadDashboard(false);resetAutoRefreshClock();} }
console.info('MyFinance app-v14 loaded — auto growth + watchlist pulse enabled');
init();
