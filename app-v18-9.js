...
function relayRequestId(){
  return `mfrelay_${Date.now()}_${Math.random().toString(36).slice(2,10)}`;
}
function relayCallbackUrl(){
  return new URL('relay.html', window.location.href).href.split(/[?#]/)[0];
}
function jsonpCallbackName(){
  return `__mfJsonp_${Date.now()}_${Math.random().toString(36).slice(2,9)}`.replace(/[^A-Za-z0-9_$]/g,'_');
}
function loadRelayResult(rid,requestId,timeoutMs){
  return new Promise((resolve,reject)=>{
    ...
    window[cb]=envelope=>{
      if(settled)return;
      if(!envelope||envelope.__myfinanceRelayResult!==true){
        fail('The backend returned an invalid relay response.');
        return;
      }
      if(envelope.requestId && envelope.requestId!==requestId){
        fail('The backend response did not match this request.');
        return;
      }
      settled=true;
      cleanup();
    ...
    u.searchParams.set('action','relayResult');
    u.searchParams.set('rid',rid);
    u.searchParams.set('callback',cb);
    u.searchParams.set('_',String(Date.now()));
    script.src=u.toString();
    script.async=true;
    script.onerror=()=>fail('The backend result could not be loaded. Please try again.');
    const timer=setTimeout(()=>fail('The backend result timed out. Please try again.'),Math.min(timeoutMs,30000));
    document.head.appendChild(script);
  });
}
function apiViaRelay(action,payload={},options={}){
  ...
  const body={action,...payload,transport:'relay',requestId,callback:relayCallbackUrl()};
  ...
  return new Promise((resolve,reject)=>{
    ...
    frame.id=frameName;
    frame.title='MyFinance backend relay';
    ...
    let settled=false;
    const cleanup=()=>{
      window.removeEventListener('message',onTicket);
      clearTimeout(timer);
    ...
    const onTicket=async event=>{
      if(event.origin!==window.location.origin)return;
      const data=event.data||{};
      if(!data.__myfinanceRelayTicket||data.requestId!==requestId||!data.rid)return;
      if(settled)return;
      settled=true;
      cleanup();
      try{
        resolve(await loadRelayResult(String(data.rid),requestId,timeoutMs));
      }catch(err){
        reject(err);
      }
    };

    window.addEventListener('message',onTicket);
    const timer=setTimeout(
      ()=>fail('The backend relay did not return. Confirm relay.html is uploaded and Backend v3.13.0 is deployed.'),
      timeoutMs
    );

    document.body.appendChild(frame);
    document.body.appendChild(form);
    try{form.submit();}catch(e){fail(`Could not send the backend request: ${e.message}`);}
  });
}
async function api(action,payload={},options={}){
  try{
    return await apiViaRelay(action,payload,options);
  }catch(error){
...
async function loadBackendVersion(){
  ...
  try{
    ...
    if(data&&data.version){
      ...
      if(parts[0]<3||(parts[0]===3&&parts[1]<13)){
        showRuntimeWarning(`Backend v${state.backendVersion} is older than required v3.13.0. Update the Apps Script backend before using V18.9.`);
      }
    }
  }catch(e){
...
console.info('MyFinance v18.9 loaded — ticketed relay + JSONP backend transport');
