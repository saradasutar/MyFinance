...
function loadSavedUsername(){
  ...
  if(els.loginVersion)els.loginVersion.textContent=`${CONFIG.APP_NAME||'My Finance'} · v${APP_VERSION}`;
  if(els.dashboardVersion)els.dashboardVersion.textContent=`${CONFIG.APP_NAME||'My Finance'} · v${APP_VERSION}`;
  if(els.dashboardVersionTop)els.dashboardVersionTop.textContent=`v${APP_VERSION}`;
}
...
function showApp(){els.loginView.classList.add('hidden');els.appView.classList.remove('hidden');els.sideAppName.textContent=CONFIG.APP_NAME||'My Finance';if(els.dashboardVersion)els.dashboardVersion.textContent=`${CONFIG.APP_NAME||'My Finance'} · v${APP_VERSION}`;if(els.dashboardVersionTop)els.dashboardVersionTop.textContent=`v${APP_VERSION}`;els.todayLabel.textContent=new Intl.DateTimeFormat('en-IN',{weekday:'long',day:'numeric',month:'long'}).format(new Date()).toUpperCase();}
...
function renderWatchlist(){
  ...
  els.watchBody.innerHTML=items.map(x=>`<tr class="watch-row" data-view-watch="${escapeHtml(x.id)}" tabindex="0" aria-label="View watchlist details for ${escapeHtml(x.assetName)}">
    <td><div class="asset-cell"><div class="asset-badge ${String(x.type).toLowerCase()}">${escapeHtml(x.type==='MF'?'MF':x.type==='ETF'?'ET':x.type==='STOCK'?'ST':'OT')}</div><div><strong>${escapeHtml(x.assetName)}</strong><span>${escapeHtml(x.exchange?`${x.exchange}:`:'')}${escapeHtml(x.code)}</span></div></div></td>
...
function openWatchDrawer(item){
  if(!item)return;
  state.drawerWatchId=item.id;
  state.drawerHoldingId='';
  state.drawerMode='WATCH';
  const isMf=item.type==='MF';
  const currentLabel=isMf?'Current NAV':'Current price';
  const targetLabel=isMf?'Target NAV':'Target buy price';
  const badge=isMf?'MF':item.type==='ETF'?'ETF':item.type==='STOCK'?'ST':'☆';
  const status=watchTargetStatus(item);
  if(els.drawerEyebrow)els.drawerEyebrow.textContent='WATCHLIST DETAILS';
  els.drawerEditBtn.textContent='Edit watch item';
  els.drawerAssetBadge.textContent=badge;
  els.drawerTitle.textContent=item.assetName||'Watchlist item';
  ...
  els.drawerContent.innerHTML=`
    <section class="drawer-value-card watch-drawer-value">
      <span class="drawer-value-label">${currentLabel}</span>
      <strong class="drawer-current-value">${formatCurrency(item.currentPrice)}</strong>
      <div class="drawer-gain-line">
        <span>${targetLabel}: ${formatCurrency(item.targetPrice)}</span>
        <span class="drawer-gain-chip ${status.cls}">${escapeHtml(status.text)}</span>
      </div>
    </section>
    <section class="drawer-section">
      <h3 class="drawer-section-title">Watch summary</h3>
      <div class="drawer-metric-grid">
        ${drawerMetric(currentLabel,formatCurrency(item.currentPrice))}
        ${drawerMetric(targetLabel,formatCurrency(item.targetPrice))}
        ${drawerMetric('Distance from target',formatPercent(item.distancePct),Number(item.distancePct)<=0?'positive':'neutral')}
        ${drawerMetric('Priority',escapeHtml(item.priority||'MEDIUM'))}
        ${drawerMetric('Manual price',formatCurrency(item.manualPrice))}
        ${drawerMetric('Price source',escapeHtml(item.priceSource||'Pending'))}
      </div>
    </section>
    <section class="drawer-section">
      <h3 class="drawer-section-title">Watchlist information</h3>
      <div class="drawer-info-list">
        <div class="drawer-info-row"><span>Asset type</span><strong>${escapeHtml(item.type||'—')}</strong></div>
        <div class="drawer-info-row"><span>${isMf?'AMFI code':'Symbol / code'}</span><strong>${escapeHtml(item.code||'—')}</strong></div>
        ${item.exchange?`<div class="drawer-info-row"><span>Exchange</span><strong>${escapeHtml(item.exchange)}</strong></div>`:''}
        <div class="drawer-info-row"><span>Priority</span><strong><span class="priority ${String(item.priority||'MEDIUM').toLowerCase()}">${escapeHtml(item.priority||'MEDIUM')}</span></strong></div>
        <div class="drawer-info-row"><span>Price date</span><strong>${detailDate(item.priceDate)}</strong></div>
        <div class="drawer-info-row"><span>Target status</span><strong class="${status.cls}">${escapeHtml(status.text)}</strong></div>
      </div>
    </section>
    <section class="drawer-section watch-note-editor-section">
      <div class="drawer-section-head">
        <h3 class="drawer-section-title">Personal Note</h3>
        <span class="tiny muted">View and edit here</span>
      </div>
      <textarea id="drawerWatchNote" class="drawer-note-textarea" rows="6" maxlength="500" placeholder="Add your personal note for this watchlist item…">${escapeHtml(item.notes||'')}</textarea>
      <div class="drawer-note-actions">
        <span id="drawerWatchNoteStatus" class="tiny muted">${item.notes?'Existing note shown above.':'No note yet.'}</span>
        <button id="drawerSaveWatchNoteBtn" class="primary-button compact-button" type="button">Save Note</button>
      </div>
    </section>
  `;
  els.holdingDrawer.classList.add('open');
  ...
  els.holdingDrawerBackdrop.setAttribute('aria-hidden','false');
  document.body.classList.add('drawer-open');
  const noteBtn=$('drawerSaveWatchNoteBtn');
  if(noteBtn)noteBtn.addEventListener('click',saveWatchDrawerNote);
  setTimeout(()=>els.drawerCloseBtn?.focus(),50);
}
...
async function init(){bindEvents();els.diaryBrowseDate.value=localIsoDate();els.diaryBrowseMonth.value=localIsoMonth();els.diaryDate.value=localIsoDate();els.monthlyEntryMonth.value=localIsoMonth();refreshMonthlyYearFilter();els.monthlyYearFilter.value=String(new Date().getFullYear());els.monthlyMonthFilter.value=String(new Date().getMonth()+1).padStart(2,'0');loadSavedUsername();refreshOwnerControls();startAutoRefresh();if(!isConfigured()){els.loginMessage.textContent='Setup required: paste the Apps Script /exec URL into config.js.';return;}if(state.token){showApp();loadCache();await loadDashboard(false);resetAutoRefreshClock();}}
console.info('MyFinance v15.3 loaded — monthly diary/plan/experience/targets enabled');
init();
