...
function apiFrameId(){
  return `mfapi_${Date.now()}_${Math.random().toString(36).slice(2,10)}`;
}
function apiViaIframe(action,payload={},options={}){
  ...
  const requestId=apiFrameId();
  const body={action,...payload,transport:'iframe',requestId};
  ...
  return new Promise((resolve,reject)=>{
    ...
    const onMessage=event=>{
      ...
      if(!data||data.__myfinanceApi!==true||data.requestId!==requestId)return;
      if(settled)return;
      settled=true;
      cleanup();
    ...
    const timer=setTimeout(()=>{
      finishError('The backend did not respond. Confirm the Apps Script Web App is deployed as Execute as Me / Anyone, then try again.');
    },timeoutMs);

    document.body.appendChild(frame);
    document.body.appendChild(form);
    try{form.submit();}catch(e){finishError(`Could not contact the backend: ${e.message}`);}
  });
}
async function api(action,payload={},options={}){
  try{
    return await apiViaIframe(action,payload,options);
  }catch(error){
  ...
  try{
    ...
    if(data&&data.version){
      ...
      if(parts[0]<3||(parts[0]===3&&parts[1]<10)){
        showRuntimeWarning(`Backend v${state.backendVersion} is older than required v3.10.0. Update the Apps Script backend before using V18.5.`);
      }
    }
  }catch(e){
...
console.info('MyFinance v18.6 loaded — larger bold Holdings Summary typography');
