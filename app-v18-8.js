...
function apiViaRelay(action,payload={},options={}){
  ...
  return new Promise((resolve,reject)=>{
    ...
    const onMessage=event=>{
      ...
      if(!data.__myfinanceRelay||data.requestId!==requestId)return;
      if(settled)return;
      settled=true;
      cleanup();
    ...
    const timer=setTimeout(
      ()=>fail('The backend relay did not return. Confirm relay.html is uploaded and the Apps Script Web App is deployed as Execute as Me / Anyone.'),
      timeoutMs
    );
  ...
  try{
    ...
    if(data&&data.version){
      ...
      if(parts[0]<3||(parts[0]===3&&parts[1]<12)){
        showRuntimeWarning(`Backend v${state.backendVersion} is older than required v3.12.0. Update the Apps Script backend before using V18.8.`);
      }
    }
  }catch(e){
...
console.info('MyFinance v18.8 loaded — same-origin relay backend transport');
